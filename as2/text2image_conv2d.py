...
from tensorflow.keras.models import Sequential, Model
from tensorflow.keras.layers import LSTM, RepeatVector, Dense, Reshape, Conv2DTranspose, BatchNormalization, Input

def build_text2image_model():
    # Text input
    text_input = Input(shape=(max_query_length, len(unique_characters)))

    # Encoder (LSTM) with 1024 units
    encoded_text = LSTM(1024)(text_input)

    # Reshape to prepare for deconvolutional layers
    reshaped_text = RepeatVector(7 * 7)(encoded_text)
    reshaped_text = Reshape((7, 7, 1024))(reshaped_text)  
    ...
    text_to_image_model = Model(inputs=text_input, outputs=generated_image)  # Use the generated_image directly as  output
...
text2image_model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['accuracy'])

# Fit the model to the training data
text2image_model.fit(X_train_text, y_train_img, epochs=100, batch_size=64)
...
# Make predictions
pred = text2image_model.predict(X_test)
